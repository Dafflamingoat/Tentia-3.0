...
function loadTimelineData() {
  ...
  if (!data) {
    console.warn('[Tentia] Aucune donnée timeline trouvée pour :', timelineId);
    return;
  }
  ...
  Object.entries(levelRewards).forEach(([lvl, rewards]) => {
    rewards.forEach(r => { rewardUnlockLevels[r.id] = parseInt(lvl, 10); });
  });
...
function getTimelineAwareRewards(levelReached) {
  // Les récompenses viennent directement du fichier timeline chargé
  return levelRewards[levelReached] || [];
}

// rewardUnlockLevels est construit dans loadTimelineData() et stocké en localStorage
const rewardUnlockLevels = JSON.parse(localStorage.getItem('_levelRewardsMap') || '{}');
